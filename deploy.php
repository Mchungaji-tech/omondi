<?php
/**
 * cPanel Deployment Script for Beacon Gospel Centre
 * Upload this file to /home/tektxbzg/morris/ and run it once via browser.
 * It creates .env and .htaccess, then runs the database installer.
 */

$basePath = '/home/tektxbzg/morris';
$envPath = $basePath . '/.env';
$htaccessPath = $basePath . '/.htaccess';
$schemaPath = $basePath . '/schema.sql';

// Auto-detect DB credentials from cPanel environment if available
...
if (file_put_contents($envPath, $envContent) !== false) {
    ...
    $status[] = ['type' => 'error', 'msg' => "Failed to create .env at $envPath. Check permissions."];
}
...
if (file_put_contents($htaccessPath, $htaccessContent) !== false) {
    ...
    $status[] = ['type' => 'error', 'msg' => "Failed to create .htaccess at $htaccessPath. Check permissions."];
}

// Step 3: Run database installer if schema.sql exists
if (file_exists($schemaPath)) {
    ...
    if ($conn) {
        ...
        $status[] = ['type' => 'error', 'msg' => "Could not connect to MySQL: " . mysqli_connect_error()];
    }
} else {
...
// Step 5: Clean up - delete deploy.php itself for security
...
<title>Deployment — Beacon Gospel Centre</title>
<style>
:root{--paper:#F7F2E9;--wine:#8C1D2F;--pine:#274D3D;--gold:#C99B3F;--line:rgba(25,22,19,.18);--ink:#191613;--ink2:#4A4238}
...
.creds{background:#17120E;color:#F7F2E9;padding:20px;border-radius:6px;margin:24px 0;font-size:13px;line-height:1.9}
.creds b{color:#E3BE6E}
...
<body>
<div class="wrap">
  <h1>cPanel Deployment Complete</h1>
  <p style="color:var(--ink2);margin-bottom:20px;">Beacon Gospel Centre has been deployed to <code>/home/tektxbzg/morris</code></p>
  ...
  <div class="creds">
    ...
    URL: <b>https://tektxbzg.com/morris/admin/</b><br>
  ...
  <div class="actions">
    <a href="/morris/" class="btn">View Website</a>
    <a href="/morris/admin/" class="btn gold">Admin Console</a>
  </div>
</div>
</body>
</html>
